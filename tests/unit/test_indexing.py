...
# ──────────────────────────────────────────────────────────────────────────────
# F-IDX-B05 — '.markdown' extension is indexable   [BUG B-05 — FIXED]
#
# Originally `.markdown` was dispatched in load_file but missing from
# SUPPORTED_EXTENSIONS, so files with that extension were silently ignored.
# Fix added '.markdown' to SUPPORTED_EXTENSIONS — regression watch.
# ──────────────────────────────────────────────────────────────────────────────
def test_F_IDX_B05_markdown_extension_is_indexable(isolated_env):
    """A .markdown file should be indexable."""
    from tests.helpers import sample_files as builders
    rag = isolated_env.rag
