...
# ──────────────────────────────────────────────────────────────────────────────
# F-TRK-05 — backup-restore (older mtime, different size)   [EXERCISES BUG B-02]
# ──────────────────────────────────────────────────────────────────────────────
@pytest.mark.xfail(
    reason="Bug B-02: directory branch uses 'mtime > old', misses older-mtime restores",
    strict=True,
)
def test_F_TRK_05_backup_restore_older_mtime(isolated_env, small_text_file):
    ...
    baseline but whose content (and size) is different. The directory
    branch's `mtime > old_mtime` check misses this entirely. The file is
    silently treated as UNCHANGED and never re-indexed.

    Fix: comparison should use abs(delta) > 1.0 OR a size check (or both).
    """
    rag = isolated_env.rag
