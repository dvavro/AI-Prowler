...
# ──────────────────────────────────────────────────────────────────────────────
# F-UPD-07 — clear_database leaves orphan state   [BUG B-04]
# ──────────────────────────────────────────────────────────────────────────────
@pytest.mark.slow
@pytest.mark.xfail(
    reason="Bug B-04: clear_database does not wipe email_index.json or auto_update list",
    strict=True,
)
def test_F_UPD_07_clear_database_wipes_all_state(isolated_env, mbox_file):
    ...
    AND the auto-update list. Currently it only does the first two — leaving
    re-indexed mbox files silently empty (UIDs still 'known') and tracked-dir
    listbox showing folders with zero chunks."""
    rag = isolated_env.rag
