...
# ──────────────────────────────────────────────────────────────────────────────
# Bug B-03: remove_directory_from_index must return 'files_removed'
# ──────────────────────────────────────────────────────────────────────────────
@pytest.mark.slow
@pytest.mark.xfail(
    reason="Bug B-03: remove_directory_from_index docstring promises files_removed "
           "but the return dict omits it",
    strict=True,
)
def test_B_03_remove_returns_files_removed_key(isolated_env):
    """Function docstring promises {chunks_removed, files_removed, errors}
    but only returns {chunks_removed, errors}. The MCP tool depends on
    files_removed and currently always reports 'unknown'."""
    from tests.helpers import sample_files as builders
    rag = isolated_env.rag
...
# ──────────────────────────────────────────────────────────────────────────────
# Bug B-08: directory branch reports files_removed == 1 regardless of count
# ──────────────────────────────────────────────────────────────────────────────
@pytest.mark.slow
@pytest.mark.xfail(
    reason="Bug B-08: directory-branch files_removed counter uses an empty-set "
           "comprehension that always falls through to 1",
    strict=True,
)
def test_B_08_directory_files_removed_counter_correct(isolated_env):
    ...
    files_removed == 5 (not 1). Depends on B-03 being fixed first since
    we can't even check files_removed without it being in the return dict."""
    from tests.helpers import sample_files as builders
    rag = isolated_env.rag
