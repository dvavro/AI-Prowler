...
def test_G_IDX_04_prescan_reports_findings(gui, isolated_env):
    """Click Scan Queue with one folder containing 2 files → output box
    eventually contains a scan report mentioning both files. No ChromaDB
    writes occur (pre-scan is read-only)."""
    ...
    gui.app._run_prescan()

    # Scan runs in a background thread, posts results to output_queue.
    # We need to wait for the worker AND for the GUI's queue-pump to flush
    # those results into the text widget. The wait_until predicate looks
    # at the actual text widget contents.
    def has_scan_report():
        text = gui.app.index_output.get("1.0", "end")
        return "scan" in text.lower() or "FILE SCAN" in text or "to be indexed" in text.lower()

    gui.wait_until(has_scan_report, timeout_s=30.0)

    output = gui.app.index_output.get("1.0", "end")
    # No assertion that content includes specific filenames — the prescan
    # output format is summary-level. The presence of a recognisable header
    # is enough to confirm the worker ran successfully.
