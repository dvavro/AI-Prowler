...
def test_G_MCP_01_add_and_index_directory(mcp_env, sample_files):
    ...
    # The folder should now be in the auto-update list
    auto_update = json.loads(mcp_env.auto_update.read_text(encoding="utf-8"))
    folder_norm = mcp_env.rag.normalise_path(str(folder.resolve()))
    assert folder_norm in auto_update["directories"], (
...
def test_G_MCP_02_add_and_index_single_file(mcp_env, small_text_file):
    ...
    auto_update = json.loads(mcp_env.auto_update.read_text(encoding="utf-8"))
    file_norm = mcp_env.rag.normalise_path(str(small_text_file.resolve()))
    assert file_norm in auto_update["directories"], (
...
def test_G_CRO_01_rag_writes_visible_to_mcp(mcp_env, sample_files):
    ...
    assert mcp_env.rag.normalise_path(str(folder.resolve())) in list_out, (
